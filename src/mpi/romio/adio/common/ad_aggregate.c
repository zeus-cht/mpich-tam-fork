...
/* +1 to avoid a 0-size malloc */
    ...
    /* 0. Let local aggregators know data size first. */
    send_total_size = 0;
    for (i = 0; i < fd->hints->cb_nodes; ++i) {
        /* Skip self-send. */
    ...
    j = 0;
    if (fd->is_agg) {
        //contig_buf = ADIOI_Malloc(memLen * 2 * sizeof(ADIO_Offset));
        ...
        //buf_ptr = (char*) contig_buf;
        buf_ptr = (char*) others_req[0].offsets;
        for ( i = 0; i < fd->local_aggregator_size; ++i ) {
        ...
        for ( i = 0; i < fd->local_aggregator_size; ++i ) {
            ...
            if (fd->local_aggregators[i] != myrank){
                ...
                for ( k = 0; k < fd->local_aggregator_domain_size[i]; ++k ) {
                    /* Local metadata has been unpacked at the beginning, nothing should be received from its local aggregator. */
                    if (fd->local_aggregator_domain[i][k] != myrank) {
                        memcpy(others_req[fd->local_aggregator_domain[i][k]].offsets, buf_ptr, 2 * others_req[fd->local_aggregator_domain[i][k]].count * sizeof(ADIO_Offset));
                        //MEMCPY_UNPACK(fd->local_aggregator_domain[i][k], (char *) buf_ptr);
                        buf_ptr += 2 * others_req[fd->local_aggregator_domain[i][k]].count * sizeof(ADIO_Offset);
                    }
                }
            }
        }
...
/* +1 to avoid a 0-size malloc */
    ...
    ptr = contig_buf;
    for (i = 0; i < nprocs; i++) {
        for (k = 0; k < others_req[i].count; ++k) {
            if (others_req[i].offsets[k] != *ptr) {
                printf("rank = %d, others_req[%d].offsets=%llu!=%llu\n", myrank, i, others_req[i].offsets[k], *ptr);
            }
            ptr++;
        }
        for (k = 0; k < others_req[i].count; ++k) {
            if (others_req[i].lens[k] != *ptr) {
                printf("rank = %d, others_req[%d].lens=%llu!=%llu\n", myrank, i, others_req[i].lens[k], *ptr);
            }
            ptr++;
        }
    }
