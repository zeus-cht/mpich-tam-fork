...
/* exchange recv_size info so that each process knows how much to
    ...
    *hole = 0;
    ...
    if (fd->atomicity) {
        ...
        j = 0;
        for (i = 0; i < nprocs; i++) {
            if (recv_size[i] > 0 && i != myrank) {
                MPI_Irecv(write_buf, 1, recv_types[j], i, myrank + i + 100 * iter,
                          fd->comm, requests + j);
                j++;
            }
        }
   ...
   user buf at location given by buf_idx. else use send_buf. */
    ...
    if (buftype_is_contig) {
        ...
        ADIOI_Fill_send_buffer(fd, buf, flat_buf, send_buf,
                               ...
                               iter, buftype_extent, write_buf, self_recv_type);
    ...
    if (fd->atomicity) {
        ...
        for (i = 0; i < nprocs; i++) {
            if (recv_size[i] > 0 && i != myrank) {
                MPI_Recv(write_buf, 1, recv_types[j++], i, myrank + i + 100 * iter,
                         fd->comm, &status);
            }
        }
    }
...
void ADIOI_Fill_send_buffer(ADIO_File fd, void *buf, ADIOI_Flatlist_node
                            ...
                            * offset_list, ADIO_Offset * len_list, int *send_size,
                            ...
                            void *write_buf, MPI_Datatype self_recv_type)
{
        ...
        while (rem_len != 0) {
            ...
            if (send_buf_idx[p] < send_size[p]) {
                if (curr_to_proc[p] + len > done_to_proc[p]) {
                    ...
                    ADIOI_BUF_COPY}
                    if (send_buf_idx[p] == send_size[p]) {
                        if (p == myrank) {      /* send to self uses MPI_Unpack() */
                            int position = 0;
                            MPI_Unpack(send_buf[p], send_size[p], &position,
                                       write_buf, 1, self_recv_type, MPI_COMM_SELF);
                        } else {
